...
def api_scan(req: ScanRequest):
    ...
    # 1. Prioritize active PC Worker (fastest, residential IP bypasses all Cloudflare bot blocks)
    active, worker_url = is_worker_active()
    if active and worker_url:
        try:
            resp = requests.post(f'{worker_url}/api/scan', json=req.dict(), timeout=8.0)
